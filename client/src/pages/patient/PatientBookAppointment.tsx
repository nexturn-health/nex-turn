                ...
                <>
                  ...
                  {!states.loading &&
                    ...
                    !states.items.length && (
                      <p className="pb-empty">
                        No booking locations are available yet. Please contact your hospital.
                      </p>
                    )}
                  ...
                  {state &&
                    ...
                    !districts.items.length && (
                      <p className="pb-empty">
                        No districts are available for this state. Try another location.
                      </p>
                    )}
...
function BookingStyles() {
  return (
    <style>{`
      ...
      .pb-popup-backdrop { position: fixed; inset: 0; z-index: 9999; display: grid; place-items: center; padding: 18px; background: rgba(15, 23, 42, .48); backdrop-filter: blur(5px); }
      .pb-popup { position: relative; width: min(100%, 430px); padding: 26px; border-radius: 20px; background: #fffaf7; border: 1px solid #f1d3c9; box-shadow: 0 24px 70px rgba(15, 23, 42, .24); text-align: center; animation: pb-popup-in .16s ease-out; }
      .pb-popup-close { position: absolute; right: 14px; top: 14px; display: grid; place-items: center; width: 38px; height: 38px; border-radius: 999px; border: 1px solid #efd7cf; background: #fff; color: #995140; }
      .pb-popup-icon { display: inline-grid; place-items: center; width: 62px; height: 62px; border-radius: 18px; background: #fff1eb; color: #b4533f; }
      .pb-popup h2 { margin-top: 16px; color: #743b30; font-size: 21px; font-weight: 700; }
      .pb-popup p { margin-top: 10px; color: #875246; font-size: 14px; line-height: 1.7; }
      .pb-popup-button { width: 100%; margin-top: 22px; }

      @keyframes pb-spin { to { transform: rotate(360deg); } }
      @keyframes pb-popup-in { from { opacity: 0; transform: translateY(10px) scale(.98); } to { opacity: 1; transform: translateY(0) scale(1); } }
      ...
      @media (max-width: 639px) {
        ...
        .pb-popup { padding: 24px 18px 20px; border-radius: 18px; }
      }

      @media (prefers-reduced-motion: reduce) {
        .pb-spin { animation: none; }
        .pb-option { transition: none; }
        .pb-popup { animation: none; }
      }
    `}</style>
  );
}